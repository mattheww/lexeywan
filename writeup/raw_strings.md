...
> an attempt to match one of the parsing expressions written as `HASHES²` behaves like an attempt to match an exact string containing the characters consumed by the (only) match of the expression written as `HASHES¹` under the same match attempt of a tokenisation nonterminal.
