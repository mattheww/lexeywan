...
///
/// Reports PriorityViolation if any lower-priority rule succeeded and matched as many, or more,
/// characters (unless it's a known exception to the longest-match principle).
/// (This is checking what "open questions" claims about a longest-match-based formulation.)
...
enum LexOutcome {
    ...
    /// Multiple rules matched, the sequence matched by the highest priority rule wasn't longer than
    /// the sequence matched by all other rules, and the situation isn't a known exception to the
